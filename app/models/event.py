...
class Event(TimestampedUUIDModel):
    ...
    provider: Mapped[EventProvider | None] = mapped_column(
        Enum(EventProvider, native_enum=False, length=32, values_callable=lambda enum_cls: [item.value for item in enum_cls]),
        nullable=True,
    )
